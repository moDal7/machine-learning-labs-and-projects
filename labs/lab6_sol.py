...
# usual mcol function to transform a row vector into a column vector
def mcol(v):
    return v.reshape((v.size, 1))

# function to load the data from the textual files
def load_data():

    lInf = []

    f=open('lab6_data/  inferno.txt', encoding="ISO-8859-1")
    ...
    f=open('lab6_data/purgatorio.txt', encoding="ISO-8859-1")
    ...
    f=open('lab6_data/paradiso.txt', encoding="ISO-8859-1")
...
# data splitting function, that appends a line every n of an array l to the test list, and the rest to the train list
def split_data(l, n):
...
def S1_compute_logLikelihoodMatrix(h_clsLogProb, lTercets, hCls2Idx = None):
    ...
    h_clsLogProb is the dictionary of model parameters as returned by S1_estimateModel
    lTercets is a list of tercets (list of strings)
    hCls2Idx: map between textual labels (keys of h_clsLogProb) and matrix rows. If not provided, automatic mapping based on alphabetical order is used
